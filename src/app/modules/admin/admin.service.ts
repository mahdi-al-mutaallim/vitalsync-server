...
const calculate_pagination = (options: {
	page?: number;
	limit?: number;
	sort?: string;
	order?: string;
	...
	const page: number =
		Number(options.limit) && Number(options.page) ? Number(options.page) : 1;
	const limit: number =
		Number(options.limit) && Number(options.page) ? Number(options.limit) : 10;
